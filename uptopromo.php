...
Description: Plugins uptopromo untuk publisher di Indonesia membantu menginstall PHP kode dari slot iklan di dalam website wordpress hanya dengan 3 klik saja - dan dapatkan pendapatan secara otomatis dengan http://www.uptopromo.co.id
Author: ipnino.ru
...
* Base settings
**/
define('PR_API_URL', 'http://uptopromo.co.id/api/');
...
class PromoCore
{
	...
	public function options()
	{
		...
		<?php } else { ?>	
			...
			<p>Pada halaman ini Anda harus memilih tema dari situs Anda dan menambahkan nya pada sistem Uptopromo untuk memulai mendapatkan penghasilan. Harap diperhatikan, bahwa platform tidak bisa ditambahkan pada akun Anda! Apabila Anda sudah menambahkan nya, silahkan membuang situs dari <a href="http://uptopromo.co.id/id/platforms">daftar platform</a> dan menambah kembali pada halaman ini dengan plugin!
			Setelah memasukan, silahkan mengecek email Anda mengenai platform untuk menyetujui atau melakukan penawaran pendapatan untuk situs Anda. Apabila ada pertanyaan mengenai hal ini, silahkan menghubungi kami di: support@uptopromo.co.id atau melalui layanan Telpon/sms/whatsapp: 081291993399</p>
		    <form method="post" action="">
		 	...
		 	<?php endif; ?>
			<?php } ?>	
			<a style="float: right;" href="http://uptopromo.co.id/id/platforms"><img src="<?php echo plugins_url('logo.jpg', __FILE__); ?>" /></a>
		</div>
		<?php
	}
		    ...
		    <form method="post" action="" >
		    	...
		    	<p class="submit">
					<input type="submit" name="update" class="button-primary" value="Log In"> atau 
					 &nbsp;<a href="http://uptopromo.co.id/id/users/sign_up" target="_blank">Buat Akun</a>
	...
	public function payments()
	{
		...
		<div class="wrap">
			...
			<?php else:?>
		    <form method="post" action="" >
		    	<a style="color: red;" href="http://uptopromo.co.id/id/users/payment" target="_blank"><?php $this->print_errors()?></a>
